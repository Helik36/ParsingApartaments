...
url = "https://www.avito.ru/syktyvkar/komnaty/prodam-ASgBAgICAUSQA7wQ?cd=1&localPriority=1&s=104&user=1"
...
async def pars_html():
    ...
    try:
        ...
        with open(f"3Rooms/komnaty_page_1.html", "w", encoding='utf-8') as file:
            file.write(html)
        print(f"Скачал страницу комнат - 1")

        with open(f"3Rooms/komnaty_page_1.html", "r", encoding='utf-8') as file:
            page = file.read()
        ...
        for i in range(1, int(last_page)):
            ...
            try:
                ...
                print(f"Скачал страницу комнат - {i + 1}")

                with open(f"3Rooms/komnaty_page_{i + 1}.html", "w", encoding='utf-8') as file:
                    file.write(html)
            except:
...
async def get_rooms_url(context: ContextTypes.DEFAULT_TYPE):

    pages = await pars_html()

    urls_from_db, date_from_db = await check_url_in_db("urls_rooms_db")
    for number_page in range(0, int(pages)):

        with open(f"3Rooms/komnaty_page_{number_page + 1}.html", "r", encoding='utf-8') as file:
            page = file.read()
        ...
        for new_url in new_urls:
            if new_url not in urls_from_db:
                print(f"Добавил комнату - {new_url}")
                await append_urls(new_url, "urls_rooms_db")
                await context.bot.send_message(chat_id=context.job.chat_id, text=f"🏝 Новая комната - {new_url}")

...
async def pars_html():
    ...
    try:
        ...
        try:
            os.mkdir("1apartaments")
        except FileExistsError:
            pass

        with open(f"1apartaments/kvartiry_syktyvkar_page_1.html", "w", encoding='utf-8') as file:
            file.write(html)
        print(f"Скачал страницу квартир - 1")

        with open(f"1apartaments/kvartiry_syktyvkar_page_1.html", "r", encoding='utf-8') as file:
            page = file.read()
        ...
        for i in range(1, int(last_page)):
            ...
            try:
                ...
                print(f"Скачал страницу квартир - {i + 1}")

                with open(f"1apartaments/kvartiry_syktyvkar_page_{i + 1}.html", "w", encoding='utf-8') as file:
                    file.write(html)
            except:
...
async def get_apartments_url(context: ContextTypes.DEFAULT_TYPE):
    ...
    for number_page in range(0, int(pages)):
        ...
        with open(f"1apartaments/kvartiry_syktyvkar_page_{number_page + 1}.html", "r", encoding='utf-8') as file:
            page = file.read()
        ...
        for new_url in new_urls:
            if new_url not in urls_from_db:
                print(f"Добавил квартиру- {new_url}")
                await append_urls(new_url, "urls_apartment_db")
